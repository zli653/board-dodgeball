...
{
  // ADD CODE
	uint8_t xl = accel_reg_read(ACCEL_OUTX_L_XL);
	uint8_t xh = accel_reg_read(ACCEL_OUTX_H_XL);
  return (xh << 8 | xl); // Modify to return the x value
...
{
  // ADD CODE
	uint8_t xl = accel_reg_read(ACCEL_OUTY_L_XL);
	uint8_t xh = accel_reg_read(ACCEL_OUTY_H_XL);
  return (xh << 8 | xl); // Modify to return the Y value
...
{
  // ADD CODE
	uint8_t xl = accel_reg_read(ACCEL_OUTZ_L_XL);
	uint8_t xh = accel_reg_read(ACCEL_OUTZ_H_XL);
  return (xh << 8 | xl); // Modify to return the Z value
